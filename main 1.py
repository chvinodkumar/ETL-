...
class DataValidation:
    ...
    def DQ_jobRun_log_to_csv(self):
        """Write the audit log to a CSV file using pandas."""
        total_records = self.passed_count + self.failed_count
        self.DQ_jobRun['status'] = f'{(self.passed_count / total_records * 100):.2f}% Passed' if total_records > 0 else 'No Records'
    ...
    def DQ_jobRun_Result_log_to_csv(self):
        """Write the audit log to a CSV file using pandas."""
        total_records = self.passed_count + self.failed_count
        self.DQ_jobRun_Result['status'] = f'{(self.passed_count / total_records * 100):.2f}% Passed' if total_records > 0 else 'No Records'
    ...
    def Rept_DQ_jobRun_Result_log_to_csv(self):
        """Write the audit log to a CSV file using pandas."""
        total_records = self.passed_count + self.failed_count
        self.Rept_DQ_jobRun_Result['status'] = f'{(self.passed_count / total_records * 100):.2f}% Passed' if total_records > 0 else 'No Records'
    ...
    def rule_1(self, val):
        ...
        failed_rows =self.source_file_df[~self.source_file_df[ource_file_dfcolumn_name].apply(lambda x: isinstance(x, (int, float)))]
    ...
    def rule_2(self, val):
        ...
        column_name = val['column_name']
        failed_rows = self.source_file_df[self.source_file_df[column_name].isnull() | (self.source_file_df[column_name] == '')]
    ...
    def rule_8(self, val):
        ...
        column_name = val['column_name']
        merged_df = pd.merge(self.source_file_df, self.manufacture_lkp_df, on=['MANUFACTURER_NAME', 'MODEL_NAME'], suffixes=('_src', '_lkp'))
    ...
    def rule_13(self, val):
        ...
        self.update_validation_status_by_index(self.manufacture_lkp_df, rule_condition.index, f"{val['column_name']}-2-Fail")
