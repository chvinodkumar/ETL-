...
class pattern_files_loader:
    ...
    def __init__(self,logger,db_config,conn_profile,audit_table,audit_schema,conn_engine,source_name,file_pattern,pattern_load_type,box_config_path=None,box_folderId=None,sftp_path=None,s3_profile=None,bucket=None,bucket_prefix=None):
        self.logger=logger
    ...
    def main(self):
       ...
       try:
        ...
        if self.source_name.lower() == 'box':
           ...
           self.logger.info(f"Exception occured while execting main method : {e}")
           raise
    ...
    def get_box_patternFiles(self):
        ...
        try:
            ...
            self.logger.info(f"pattern files searcing process has been started from source sytem box")
    ...
    def get_sftp_patternfiles(self):
        ...
        """
        try:
            self.logger.info(f"pattern files searcing process has been started from source sytem sftp")
            if self.file_pattern is None:
                self.logger.info(f"file_pattern cannot be None")
                raise ValueError("file_pattern cannot be None")
            patterns=self.file_pattern.split('*')
    ...
    def get_s3_patternfiles(self):
        ...
        """
        try:
            self.logger.info(f"pattern files searcing process has been started from source system S3")
            if self.file_pattern is None:
                self.logger.info(f"file_pattern cannot be None")
                raise ValueError("file_pattern cannot be None")
            patterns=self.file_pattern.split('*')
