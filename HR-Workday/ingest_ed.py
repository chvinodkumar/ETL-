...
def fetch_data() -> None:
    ...
    #sent_after = "2023-12-15T12:00:00.000-07:00"
...
def output_errors(errors_get: [list[dict]], errors_post: [list[dict]]) -> None:
    ...
    if errors:
        ...
        send_email_notification("PROD|HR-WORKDAY|Failed",f"Error(s) occured when ingesting following integration(s):\n{errors}")
    ...
    for s in seq:
        for file in files:
            if s in file:
                ...
                pass
    send_email_notification("PROD|HR-WORKDAY|VALIDATION", f"{len(files)}Number of files loaded to s3")
